...
eyeData_pt = append('eyeDataGAZE_', ptID,'.mat');
tempFile_name = eyeData_pt;

% Set tempFile_name
saveFile_name1 = ['clgaze_', tempFile_name];
...
for i = 1:sessSnum
    ...
    switch sessSfieldN{i}
        ...
        case 'sceneRecog'
            if ~(length(outInfo.(sessSfieldN{i}).TTLinfo) < 180)
    ...
    for eyE = 1:2
        ...
        switch eyE % LEFT EYE
            case 1
                gazeData = [outInfo.(sessSfieldN{i}).leftEYE.gaze_Pic,...
                    outInfo.(sessSfieldN{i}).leftEYE.gaze_Pic_time];

                outInfo.(sessSfieldN{i}).leftEYE.GAZEcl  =...
                    cleanEye_GAZE(gazeData,0);

                disp('Left EYE finished')
            case 2 % RIGHT EYE
                gazeData = [outInfo.(sessSfieldN{i}).rightEYE.gaze_Pic,...
                    outInfo.(sessSfieldN{i}).rightEYE.gaze_Pic_time];

                outInfo.(sessSfieldN{i}).rightEYE.GAZEcl  =...
                    cleanEye_GAZE(gazeData,0);

                disp('Right EYE finished')
