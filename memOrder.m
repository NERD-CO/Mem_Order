    ...
    methods
        ...
        function identifyTTLs(obj)
            ...
            for ii = 1:length(obj.validTasks)
                ...
                %% Load the behavior info
                obj.(task).behave_info = load(obj.(task).behave_fname);
                ...
                obj.(task).crossOnIdx = obj.(task).LFPIdx(obj.(task).eventIDs == 11);
                obj.(task).clipOnIdx = obj.(task).LFPIdx(obj.(task).eventIDs == 1);
                obj.(task).clipOffIdx = obj.(task).LFPIdx(obj.(task).eventIDs == 2);
                obj.(task).questionIdx = obj.(task).LFPIdx(obj.(task).eventIDs == 3);
                ...
                if strcmp(obj.subID,'MW26') & strcmp(task,'sceneRecog')
                    ...
                    obj.(task).crossOnIdx(127) = [];
                    obj.(task).clipOnIdx(127) = [];
                    obj.(task).clipOffIdx(127) = [];
                    obj.(task).questionIdx(127) = [];
                end
                ...
                obj.(task).crossOffIdx = obj.(task).crossOnIdx + (floor(obj.(task).fs .* cross));
                obj.(task).responseIdx = obj.(task).questionIdx + (floor(obj.(task).fs .* response));
        ...
        function allocateData(obj)
            ...
            for ii = 1:length(obj.validTasks)
                ...
                %% Grab data segments
                obj.(task).fixation = arrayfun(@(i) extractSegments(obj.(task).crossOnIdx(i),obj.(task).crossOffIdx(i)), 1:length(obj.(task).boundary), 'UniformOutput', false )';
                obj.(task).presentation = arrayfun(@(i) extractSegments(obj.(task).clipOnIdx(i),obj.(task).clipOffIdx(i)), 1:length(obj.(task).boundary), 'UniformOutput', false )';
                obj.(task).question = arrayfun(@(i) extractSegments(obj.(task).questionIdx(i),obj.(task).responseIdx(i)), 1:length(obj.(task).boundary), 'UniformOutput', false )';
                obj.(task).epoch = arrayfun(@(i) extractSegments(obj.(task).crossOnIdx(i),obj.(task).responseIdx(i)), 1:length(obj.(task).boundary), 'UniformOutput', false )';
        ...
        function baselineCorrection(obj)
            ...
            for ii = 1:length(obj.validTasks)
                ...
                %% Loop through trials
                for jj = 1:length(obj.(task).fixation)

                    %% Calculate channel means
                    ch_mean = mean(obj.(task).fixation{jj,1},2);

                    %% Subtract from fixation
                    obj.(task).fixation{jj,1} = obj.(task).fixation{jj,1} - ch_mean;

                    %% Subtract from presentation
                    obj.(task).presentation{jj,1} = obj.(task).presentation{jj,1} - ch_mean;

                    %% Subtract from question
                    obj.(task).question{jj,1} = obj.(task).question{jj,1} - ch_mean;
                end
            end
        end
        ...
        function plotPhases(obj)
            ...
            for ii = 1:length(obj.validTasks)
                ...
                bands = [obj.(task).crossOnIdx,obj.(task).crossOffIdx];
                ...
                bands = [obj.(task).clipOnIdx,obj.(task).clipOffIdx];
                xp = [bands fliplr(bands)];
                for k = 1:size(bands,1)
                ...
                %% Bands for response
                bands = [obj.(task).questionIdx,obj.(task).responseIdx];
                xp = [bands fliplr(bands)];
                for k = 1:size(bands,1)
        ...
        function computePower(obj,freq)
            ...
            for ii = 1:length(obj.validTasks)
                ...
                for jj = 1:length(obj.validPhases)
                    ...
                    %% Loop through trials
                    for kk = 1:length(obj.(task).(obj.validPhases{jj,1}))

                        %% Grab the trial data
                        tmp = obj.(task).(obj.validPhases{jj,1}){kk,1};
                        ...
                        for ll = 1:size(tmp,1)
                            ...
                            if ~isnan(tmp(ll,:)) & length(tmp) > 200
                                ...
                                obj.(task).(powerField){kk,1}(ll,:) = mean(abs(wt(I,:)),1);

                            else
                                obj.(task).(powerField){kk,1}(ll,:) = NaN;
                            end
        ...
        function computeStats(obj)
            ...
            for ii = 1:length(obj.validTasks)
                ...
                for jj = 1:length(obj.validPhases)
                    ...
                    for kk = 1:length(obj.(task).(phasePower))

                        %% Create field name
                        ave_power = append('ave_',phasePower);

                        %% Grab the data
                        tmp = obj.(task).(phasePower){kk,1};
                        ...
                        if ~any(isnan(tmp))
                            ...
                            for ll = 1:size(tmp,1)
                                obj.(task).(ave_power)(ll,kk) = mean(obj.(task).(phasePower){kk,1}(ll,:));
                            end
                        end
                    end
                end
                ...
                for jj = 1:size(obj.(task).ave_presentationPower,1)
                    obj.(task).powerStats(jj,1) = ranksum(obj.(task).ave_presentationPower(jj,:), obj.(task).ave_questionPower(jj,:));
                end

                %% Correct for false discovery using Benjamini-Hochberg
                [val,idx] = sort(obj.(task).powerStats,'ascend');
                ...
                obj.(task).powerProb = val(corrected);
                obj.(task).powerSigChannels = new_names_corrected(corrected);
        ...
        function functionalConnectivity(obj,method)
            ...
            for ii = 1:length(obj.validTasks)
                ...
                for jj = 1:length(obj.validPhases)
                    ...
                    obj.(task).fcMethod = method;

                    %% Loop through trials
                    for kk = 1:length(obj.(task).(phase))

                        %% Grab the data
                        tmp = obj.(task).(phase){kk,1};
                        ...
                        if strcmp(method,'granger')
                            ...
                            %% Preallocate
                            fc = append(phase,'FC');
                            obj.(task).(fc){kk,1} = zeros(size(tmp,1),size(tmp,1));

                            %% Skip trial if not enough data
                            if length(tmp) < 4*lags
                                obj.(task).(fc){kk,1} = NaN(size(tmp,1),size(tmp,1)); % or zeros
                                continue
                            end

                            %% Add lags to the object
                            obj.(task).numLags = lags;
                            ...
                            for ll = 1:size(tmp,1)
                                for mm = 1:size(tmp,1)
                                    if ll ~= mm
                                        ...
                                        %% Write FC values
                                        obj.(task).(fc){kk,1}(ll,mm) = log(var_reduced/var_full);
                            ...
                            for ll = 1:size(tmp,1)
                                for mm = 1:size(tmp,1)
                                    if ll ~= mm
                                        ...
                                        obj.(task).(fc){kk,1}(ll,mm) = mean(x(J));
                                    elseif ll == mm
                                        fc = append(phase,'FC');
        ...
        function analyzeFC(obj,boundary)
            ...
            for ii = 1:length(obj.validTasks)
                ...
                if isfield(obj.(task),'presentationFC') && isfield(obj.(task),'questionFC')
